...
class Frame {
    ...
    public void configureFrame() {
        ...
        pieChartPanel.setBounds(200, 15, 328, 220);
    ...
    private JPanel createExpensePanel() {
        ...
        String[] expenseCategories = { "Housing", "Transportation", "Food", "Utilities", "Healthcare", "Entertainment",
                "Savings" };
        ...
        addExpense.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                int result = JOptionPane.showConfirmDialog(null, mainPanel, "Add Expense", JOptionPane.OK_CANCEL_OPTION,
    ...
    class ExpenseManager {
        ...
        public void removeRecord(Expense expense) {
            expenses.remove(expense);
        }

        public ArrayList<Expense> pullExpenseCategory(String category) {
    ...
    private JPanel removeAddExpense() {
        ...
        JButton confirmRemovalButton = new JButton();
        confirmRemovalButton.setText("Confirm Removal");
        confirmRemovalButton.setHorizontalAlignment(JTextField.CENTER);
        confirmRemovalButton.setForeground(new Color(133, 177, 204));
        confirmRemovalButton.setFont(new Font("Arial", Font.BOLD, 13));
        confirmRemovalButton.setPreferredSize(new Dimension(160, 32));
        confirmRemovalButton.setFocusPainted(false);
        confirmRemovalButton.setFocusable(true);
        confirmRemovalButton.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));

        JDialog removeRecordsDialog = new JDialog();
        removeRecordsDialog.setTitle("Remove Records");
        removeRecordsDialog.setBounds(200, 40, 700, 500);
        removeRecordsDialog.setLayout(new BorderLayout());
        removeRecordsDialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);

        DefaultTableModel removeRecords = new DefaultTableModel(
                new Object[] { "Category", "Date", "Amount", "Description" }, 0);
        JTable removeExpenseTable = new JTable(removeRecords);

        TableRowSorter<DefaultTableModel> tableRowSorter = new TableRowSorter<>(removeRecords);
        removeExpenseTable.setRowSorter(tableRowSorter);

        String[] expenseCategories = { "All Categories", "Housing", "Transportation", "Food", "Utilities", "Healthcare",
                "Entertainment",
                "Savings" };
        JComboBox<String> selectCategoryBox = new JComboBox<>(expenseCategories);
        selectCategoryBox.setPreferredSize(new Dimension(120, 32));

        selectCategoryBox.addActionListener(e -> {
            String searchCategory = (String) selectCategoryBox.getSelectedItem();
            if (searchCategory != null) {
                if (searchCategory.equals("All Categories")) {
                    tableRowSorter.setRowFilter(null);
                } else {
                    tableRowSorter.setRowFilter(RowFilter.regexFilter("(?i)" + searchCategory, 0));
                }
            }
        });

        removeRecords.addRow(new Object[] { "Housing", "11/29/2024", "$29.12", "Rent" });
        removeRecords.addRow(new Object[] { "Entertainment", "11/29/2024", "$29.12", "Movies" });
        removeRecords.addRow(new Object[] { "Savings", "11/29/2024", "$29.12", "Bank Deposit" });
        removeRecords.addRow(new Object[] { "Transportation", "11/29/2024", "$29.12", "Gas" });
        removeRecords.addRow(new Object[] { "Utilities", "11/29/2024", "$29.12", "Electricity" });
        removeRecords.addRow(new Object[] { "Food", "11/29/2024", "$29.12", "Groceries" });

        JPanel centerConfirmPanel = new JPanel(new FlowLayout(FlowLayout.CENTER));
        centerConfirmPanel.add(new JLabel("Select a Category:"));
        centerConfirmPanel.add(selectCategoryBox);

        removeRecordsDialog.add(centerConfirmPanel, BorderLayout.NORTH);
        removeRecordsDialog.add(new JScrollPane(removeExpenseTable), BorderLayout.CENTER);

        confirmRemovalButton.addActionListener(e -> {
            int selectedRow = removeExpenseTable.getSelectedRow();
            if (selectedRow != -1) {
                int modelRow = removeExpenseTable.convertRowIndexToModel(selectedRow);
                removeRecords.removeRow(modelRow);
        ...
        JPanel bottomConfirmPanel = new JPanel(new FlowLayout(FlowLayout.CENTER));
        bottomConfirmPanel.add(confirmRemovalButton);
        removeRecordsDialog.add(bottomConfirmPanel, BorderLayout.SOUTH);
        ...
        removeExpense.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                removeRecordsDialog.setVisible(true);
 ...
 * Original Source:
 * https://1bestcsharp.blogspot.com/2024/09/java-create-pie-donut-chart.html
 * Author: 1BestCsharp blog
 * Changes were made to integrate this functionality into the Expense Tracker
 * project.
 * ------------------------------------------------------------
 */
